...
export function AdminQuizForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  // ⚡ Mutation configured with native sonner lifecycle overlays
  const mutation = trpc.adminquizathon.updateEvent.useMutation({
    onMutate: () => {
      // ⏳ Throw an immediate loading state overlay
      toast.loading("Saving configuration changes...", { id: "quiz-update-toast" });
    },
    onSuccess: () => {
      // 🎉 Resolve smoothly with confirmation context
    ...
    onError: (err) => {
      // ❌ Override toast cleanly with precise backend failure message
    ...
    <div className="space-y-4 pt-4">
      ...
      <div className="grid grid-cols-2 gap-3 pt-2">
        ...
        <Button 
          type="button"
          variant="outline"
          className="border-indigo-200 text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700 h-11 rounded-xl font-bold"
        ...
        {/* Update Event details for the current month */}
        <Button 
          type="button"
          ...
          className="bg-indigo-600 hover:bg-indigo-700 h-11 rounded-xl font-bold shadow-lg shadow-indigo-100" 
