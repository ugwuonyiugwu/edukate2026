...
export function AdminRegistrations() {
  ...
  const deleteMutation = trpc.adminquizathon.deleteRegistration.useMutation({
    onMutate: (variables) => {
      ...
      const targetName = targetReg?.userName ?? "Participant";
  ...
  return (
    <div className="bg-white border-0 md:border-2 rounded-sm overflow-hidden">
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-slate-50/50 hover:bg-transparent">
            <TableHead className="font-bold px-6 py-3">Participant</TableHead>
            <TableHead className="font-bold px-6 py-3">Exam Date</TableHead>
            <TableHead className="font-bold px-6 py-3">Subjects</TableHead>
            <TableHead className="font-bold px-6 py-3">Score</TableHead>
            <TableHead className="text-right font-bold px-6 py-3">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {registrations.map((reg) => {
            const userName = reg.userName ?? "User";
              ...
              <TableRow key={reg.id} className={`transition-colors ${isDeleting ? "opacity-40 bg-red-50/50" : "hover:bg-slate-50/30"}`}>
                ...
                <TableCell className="px-6 py-4">
                  <Link href={`/profile/${reg.clerkId}`} className="flex items-center gap-3 group">
                    <UserAvatar imageUrl={reg.imageUrl} name={userName} size={38} />
                ...
                <TableCell className="px-6 py-4">
                  <div className="flex gap-1 flex-wrap max-w-50">
                    {reg.selectedSubjects.map((s) => (
                      <span 
                        key={String(s)} 
                        className="px-2 py-0.5 border-0 text-slate-600 text-[9px] font-black uppercase"
                      >
                ...
                <TableCell className="text-right px-6 py-4">
                  <Button
                    ...
                    onClick={() => setConfirmDelete({ isOpen: true, id: reg.id, name: userName })}
                    disabled={isDeleting}
                  >
