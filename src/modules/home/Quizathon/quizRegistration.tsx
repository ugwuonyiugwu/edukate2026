...
export function SubjectSelectionForm() {
  ...
  // Consolidated alert metadata tracker 
  ...
  // SERVER-ANCHORED MASTER TIME STATE
  ...
  // Mutation hooked up to trigger toast messages natively
  ...
  if (loadingStatus) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-slate-100 max-w-2xl mx-auto shadow-sm">
  ...
  // 🛡️ Safe Early Return: Narrows the type profile of 'quiz' for structural execution below
  if (!quiz) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-slate-100 max-w-2xl mx-auto text-center shadow-sm">
  ...
  if (isRegistered) {
    return (
      <div className="bg-white rounded-sm shadow-xl border border-slate-100 overflow-hidden max-w-2xl mx-auto transition-all">
        <div className="bg-blue-600 p-8 text-white">
          <div className="flex justify-between items-center gap-4">
            ...
            <div className="text-right shrink-0 bg-blue-700/40 border border-white/10 p-3 rounded-xl min-w-[155px] flex flex-col justify-center min-h-[58px]">
              {!targetTimestamp ? (
                ...
                <div>
                  ...
                  <span className="text-sm font-mono font-black tracking-tight text-white block mt-0.5">
        ...
        <div className="p-8 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-start gap-3">
              ...
              <div>
                ...
                <span className="text-sm mt-0.5 block font-black text-slate-800">
          ...
          <div className="p-5 border border-slate-100 rounded-2xl bg-slate-50/50 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {userRegistration.selectedSubjects.map((subject, index) => (
                <div key={subject} className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
          ...
          <Button
            onClick={() => {
              if (isExamTimeLive) {
                router.push("/quizathon/exam");
              }
            }}
            disabled={!isExamTimeLive}
            className={cn(
              "w-full h-14 text-sm font-black uppercase tracking-wider rounded-2xl shadow-lg transition-all",
              isExamTimeLive 
  ...
  return (
    <div className="bg-white rounded-sm shadow-xl border border-slate-100 overflow-hidden max-w-2xl mx-auto relative">
      <div className="bg-indigo-600 p-8 text-white">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-black">{quiz.title}</h1>
            <p className="text-indigo-100 text-sm mt-1 flex items-center gap-2">
              <Info className="w-4 h-4" /> Choose your 4 core subjects
            </p>
          </div>
          <div className="text-right">
            <span className="block text-[10px] uppercase font-bold opacity-70">Entry Fee</span>
            <span className="text-xl font-black">{quiz.pointCost} PT</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-8">
        <div className="space-y-6">
            ...
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  role="combobox"
                  aria-expanded={open}
                  className="w-full justify-between h-14 rounded-2xl border-slate-200 hover:bg-slate-50 transition-all text-slate-600 targets-combobox"
              ...
              <PopoverContent className="w-[--radix-popover-trigger-width] p-0 rounded-2xl overflow-hidden" align="start">
                <Command loop>
          ...
          <div className="min-h-25 p-4 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
            {selected.length === 0 ? (
              <p className="text-slate-400 text-sm text-center py-6">Your selections will appear here</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {selected.map((s) => (
                  <Badge 
                    key={s} 
                    variant="secondary"
                    className="bg-white text-indigo-700 border border-indigo-100 px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-2 hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-all cursor-pointer group"
        ...
        <div className="mt-8 pt-6 border-t">
          ...
          <Button
            className="w-full h-14 text-lg font-black bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl shadow-lg shadow-indigo-200 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
