...
export const userQuizRouter = createTRPCRouter({
  ...
  getUserRegistrations: protectedProcedure.query(async ({ ctx }) => {
    ...
    return rawRegistrations.map((reg) => {
      ...
      if (reg.batchDate) {
        ...
        examTargetTimestamp = new Date(`${pureDatePart}T21:00:00.000Z`).getTime();
      }
  ...
  getLiveExamQuestions: protectedProcedure.query(async ({ ctx }) => {
    ...
    const examStartTimestamp = new Date(`${pureDatePart}T21:00:00.000Z`).getTime();
    const examEndTimestamp = examStartTimestamp + (2 * 60 * 1000); 
  ...
  getLeaderboard: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      console.log("Input quizEventId:", input.quizEventId);
      console.log("Current user clerkId:", ctx.user?.clerkId);

      const registrations = await ctx.db.query.quizRegistrations.findMany({
        where: eq(quizRegistrations.quizEventId, input.quizEventId),
        orderBy: [desc(quizRegistrations.score), desc(quizRegistrations.registeredAt)],
      });

      console.log(`[DEBUG] Found ${registrations.length} registrations for event ${input.quizEventId}:`, registrations);

      if (!registrations.length) {
        console.log("[DEBUG] Returning empty array because no registrations found.");
        return [];
      }

      const clerkIds = registrations.map((r) => r.clerkId);
      console.log("[DEBUG] Extracted clerkIds:", clerkIds);

      const userProfiles = await ctx.db.query.users.findMany({
        where: (usersTable, { inArray }) => inArray(usersTable.clerkId, clerkIds),
      });

      console.log(`[DEBUG] Found ${userProfiles.length} user profiles matching registrations:`, userProfiles);

      const userMap = new Map(userProfiles.map((u) => [u.clerkId, u]));

      const results = registrations.map((reg) => {
        const user = userMap.get(reg.clerkId);
        const fullName = [user?.firstName, user?.lastName].filter(Boolean).join(" ");
        const finalName = fullName || user?.username || `Participant (${reg.clerkId.slice(-4)})`;
        
        console.log(`[DEBUG] Mapping reg id ${reg.id} (Clerk: ${reg.clerkId}) -> Name: "${finalName}", Score: ${reg.score}`);
      ...
      console.log("[DEBUG] Final Leaderboard payload returned:", results);
      return results;
    }),
});