...
import { Eye, EyeOff, Loader2, User, Mail, Lock, CheckCircle2, Circle, Gift } from "lucide-react";
...
const SignUpFormContent = () => {
  ...
  const [referralInput, setReferralInput] = React.useState(initialRef);
  const [showPw, setShowPw] = React.useState(false);
  const [isLoading, setIsLoading] = React.useState(false);

  // Helper generator to produce a clean unique referral code for the new user
  const generatedOwnReferralCode = React.useMemo(() => {
    const cleanName = username ? username.replace(/[^a-zA-Z0-9]/g, "").slice(0, 6).toUpperCase() : "EDK";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await signUp.create({ 
        ...
        unsafeMetadata: { 
          role: role,
          referralCode: generatedOwnReferralCode,
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-slate-50/50 px-4 py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input 
                  className="bg-slate-50/50 border-slate-200 h-10 rounded-sm pl-12 focus:ring-2 focus:ring-blue-500/10"
                  type="text" 
            ...
            <div className="space-y-1.5">
              <Label className="text-slate-500 font-medium text-sm ml-1">Referral Code (Optional)</Label>
              <div className="relative">
                <Gift className="absolute left-4 top-3 text-slate-300 h-5 w-5" />
                <Input 
                  className="bg-slate-50/50 border-slate-200 h-10 rounded-sm pl-12 focus:ring-2 focus:ring-blue-300  font-medium text-xs"
                  type="text" 
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input 
                  className="bg-slate-50/50 border-slate-200 h-10 rounded-sm pl-12 pr-12 focus:ring-2 focus:ring-blue-500/10"
            ...
            <Button 
              ...
              className={`w-full h-10 rounded-sm font-bold shadow-lg transition-all active:scale-95 mt-4 ${
                isPasswordValid 
