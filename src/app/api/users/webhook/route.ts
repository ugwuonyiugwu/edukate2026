...
import { createClerkClient } from "@clerk/backend"; // Import the backend SDK client
import { db } from "@/db";
import { users } from "@/db/schema";
import { eq } from "drizzle-orm";

// Initialize the backend client using your secret key
...
export async function POST(req: Request) {
  ...
  if (eventType === "user.created") {
    ...
    try {
      ...
      try {
        // Purge the user from Clerk completely so they can't access any authenticated pages
      ...
      // Return a 500 error so Clerk knows the webhook delivery failed natively
