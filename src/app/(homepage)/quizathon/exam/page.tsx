...
export default async function QuizDashboardPage({
  ...
  try {
    ...
    const parsedEventId = Number(resolvedSearchParams?.eventId);

    if (!resolvedSearchParams?.eventId || isNaN(parsedEventId)) {
      throw new Error("Invalid or missing examination event ID in URL parameters.");
    }

    await trpc.userquizathon.getLiveExamQuestions.prefetch({ quizEventId: parsedEventId } as any);
    await trpc.userquizathon.getUserRegistrations.prefetch();

    const [examPayload, userRegistrations] = await Promise.all([
      trpc.userquizathon.getLiveExamQuestions({ quizEventId: parsedEventId } as any),
      trpc.userquizathon.getUserRegistrations(),
    ]);

    const { categorizedQuestions, secondsRemaining } = (examPayload ?? {}) as {
      categorizedQuestions?: Record<string, any>;
    ...
    return (
      <HydrateClient>
        <Suspense fallback={<LoadingSpinner/>}>
          <LiveExamPortal 
            quizEventId={parsedEventId}
            initialSecondsRemaining={secondsRemaining ?? 0}
